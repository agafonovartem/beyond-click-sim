...
"""Run MovieLens-1M Llama 3.3 70B interaction prediction on one TP=4 vLLM."""
...
METHOD_ORDER = (
    ...
    "agent4rec_yes_no_vllm_llama33_70b_taste_gpt4o_mini_full",
    "agent4rec_yes_no_vllm_llama33_70b_traits_full",
    "agent4rec_yes_no_vllm_llama33_70b_traits_taste_gpt4o_mini_full",
)
DEFAULT_RATIOS = (1, 3, 9, 19)
DEFAULT_SEEDS = (0, 1, 2)
DEFAULT_VLLM_CLIENTS = ("vllm_local",)
DEFAULT_MAX_WORKERS_PER_TASK = 8
DEFAULT_AGENT4REC_MAX_TOKENS = 4096
...
def main() -> None:
    ...
    if args.agent4rec_max_tokens < 1:
        raise ValueError("--agent4rec-max-tokens must be positive")

    output_root = Path(args.output_root).expanduser().resolve()
    run_id = args.run_id or datetime.now(UTC).strftime("%Y%m%dT%H%M%SZ")
    batch_root = output_root / f"{run_id}_ml1m_llama33_70b_local_tp4"
    ...
    print(f"agent4rec_max_tokens={args.agent4rec_max_tokens}", flush=True)
    if args.dry_run:
        return
    ...
    _write_batch_config(
        batch_root,
        {
            ...
            "agent4rec_max_tokens": args.agent4rec_max_tokens,
            "llama_model": LLAMA_MODEL,
            "serving": "single vLLM endpoint, tensor_parallel_size=4",
...
def _run_one_task_method(
    ...
    stale_error_path = output_dir / "run_error.txt"
    ...
    try:
        ...
        metrics = _run_method(
            ...
            agent4rec_max_tokens=agent4rec_max_tokens,
        )
        row = summarize_metrics(
            task_name=task_name,
            method_name=method_name,
...
def _run_method(
    ...
    *,
    ...
    if method_name == "llm_yes_no_vllm_llama33_70b_with_item_stats_full":
    ...
    if method_name == "agent4rec_yes_no_vllm_llama33_70b_taste_gpt4o_mini_full":
        return agent4rec_yes_no.run_method(
            ...
            max_candidate_groups=None,
            max_workers=max_workers,
            max_tokens=agent4rec_max_tokens,
    ...
    if method_name == "agent4rec_yes_no_vllm_llama33_70b_traits_full":
        return agent4rec_yes_no.run_method(
            task,
            output_dir,
            method_name=method_name,
    ...
    if method_name == "agent4rec_yes_no_vllm_llama33_70b_traits_taste_gpt4o_mini_full":
        return agent4rec_yes_no.run_method(
            task,
            output_dir,
            method_name=method_name,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--agent4rec-max-tokens",
        type=int,
        default=DEFAULT_AGENT4REC_MAX_TOKENS,
        help="Max completion tokens for Agent4Rec yes/no decisions.",
    )
    parser.add_argument(
        "--output-root",
