...
from beyond_click_sim.scorers import HistoryMeanRegressor, HistoryModeRegressor
...
HISTORY_MEAN_METHOD_NAME = "history_mean_regressor"
HISTORY_MODE_METHOD_NAME = "history_mode_regressor"


def run_history_mean(task: Task, output_dir: Path) -> dict[str, object]:
    return run_method(
        task,
        output_dir,
        method_name=HISTORY_MEAN_METHOD_NAME,
        scorer_class=HistoryMeanRegressor,
    )


def run_history_mode(task: Task, output_dir: Path) -> dict[str, object]:
    return run_method(
        task,
        output_dir,
        method_name=HISTORY_MODE_METHOD_NAME,
        scorer_class=HistoryModeRegressor,
    )
...
def run_method(
    ...
    *,
    method_name: str,
    scorer_class: type[HistoryMeanRegressor] | type[HistoryModeRegressor],
...
def _scorer_manifest(
    *,
    scorer: HistoryMeanRegressor | HistoryModeRegressor,
    ...
    if isinstance(scorer, HistoryModeRegressor):
