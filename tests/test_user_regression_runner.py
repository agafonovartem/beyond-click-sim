...
from runners.in_distribution.regression_prediction.methods.user import (  # noqa: E402
    run_user_mean,
    run_user_mode,
)


def test_user_regression_runners_write_regression_artifacts(tmp_path: Path) -> None:
    task = Task(
    ...
    mean_metrics = run_user_mean(task, mean_dir)
    mode_metrics = run_user_mode(task, mode_dir)

    for run_dir, method_name in [
        (mean_dir, "user_mean_regressor"),
        (mode_dir, "user_mode_regressor"),
    ]:
    ...
    assert mean_metrics["method"] == "user_mean_regressor"
    assert mode_metrics["method"] == "user_mode_regressor"
