...
def test_cg5_registry_keeps_old_eval1000_tasks_available() -> None:
    ...
    assert "steam_item_stats_cap20_eval_users1000_cg5_m19_seed0" in TASK_BUILDERS
...
def test_cg5_item_stats_are_not_defaults() -> None:
    assert len(EVAL1000_CG5_ITEM_STATS_TASK_BUILDERS) == 30
    assert all(
        task_name.startswith(("ml-1m_item_stats_", "steam_item_stats_"))
...
def test_eval100_cg5_tasks_are_debug_variants_not_defaults() -> None:
    ...
    assert len(EVAL100_CG5_ITEM_STATS_TASK_BUILDERS) == 30
    ...
    assert "steam_item_stats_cap20_eval_users100_cg5_m1_seed0" in TASK_BUILDERS
