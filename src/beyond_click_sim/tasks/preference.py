...
class PreferencePredictionTaskBuilder(TaskBuilder):
    """Build observed-only preference candidate sets.

    Train contains all observed interactions as user history with a binary
    preference target. Validation/test contain held-out observed
    preference-positive and preference-negative rows grouped into candidate
    sets. Unlike interaction alignment tasks, this builder does not create
    sampled non-interactions and therefore has no sampled column.

    `history_context_columns` are copied from observed train interactions only.
    They are set to missing for validation/test candidates to avoid leaking
    held-out feedback into scorer inputs.
    """
    ...
    def __init__(
        ...
        *,
        ...
        if self.dataset_filter is None:
            raise ValueError("PreferencePredictionTaskBuilder requires dataset_filter.")
        if self.splitter is None:
            raise ValueError("PreferencePredictionTaskBuilder requires splitter.")
        if self.sampler is None:
            raise ValueError("PreferencePredictionTaskBuilder requires sampler.")
        if self.candidate_group_column is None:
            raise ValueError(
                "PreferencePredictionTaskBuilder requires candidate_group_column."
            )
    ...
    def build(self, dataset: CanonicalDataset) -> Task:
        ...
        train = self._with_features(
            rows=self._observed_rows(split.train, candidate_group=pd.NA),
    ...
    def _observed_rows(
        ...
        *,
        ...
        """Convert observed rows into target rows."""
