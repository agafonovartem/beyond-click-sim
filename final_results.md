...
- The July 16-20 history-listwise and Agent4Rec summary/control rows below are backed by tracked compact manifests and metrics copied from the experiment PVCs; row-level predictions and logs remain local.
...
#### Current ML-1M listwise and Agent4Rec summary study

These July 16-20 runs use the same `ml-1m_item_stats_cap20_eval_users1000_cg5`
tasks and candidate rows across models and matched summary controls. Direct
history listwise prompts expose rating history plus train-only item rating mean
and count, without summaries. Agent4Rec uses `traits+taste`; taste text was
generated by `gpt-4o-mini` through the VK OpenAI proxy and then reused from the
persistent cache. Agent4Rec candidate prompts always expose title, train-only
item rating mean, and genres; the candidate-summary variant additionally
exposes `item_summary`.

Strict listwise NDCG@5 uses
`test_failure_as_zero_group.macro_by_user_group_mean.ndcg@5`. Values are
mean +/- sample standard deviation over seeds 0-2. Failed groups are test
candidate groups that did not produce a complete parseable ranking.

| method | backbone | candidate summary | m=1 | m=3 | m=9 | m=19 | failed groups |
|---|---|---|---:|---:|---:|---:|---:|
| History + item stats, listwise | Qwen3-8B | no | 0.7272 +/- 0.0095 | 0.5116 +/- 0.0037 | 0.4172 +/- 0.0018 | 0.3602 +/- 0.0044 | 8,206 |
| History + item stats, listwise | Qwen3.6-27B | no | 0.8886 +/- 0.0010 | 0.7130 +/- 0.0038 | 0.6639 +/- 0.0045 | 0.6011 +/- 0.0044 | 57 |
| Agent4Rec traits+taste + item stats, listwise | Qwen3-8B | yes | 0.7213 +/- 0.0033 | 0.5134 +/- 0.0045 | 0.4350 +/- 0.0040 | 0.3812 +/- 0.0085 | 4,172 |
| Agent4Rec traits+taste + item stats, listwise | Qwen3.6-27B | yes | 0.7870 +/- 0.0046 | 0.5782 +/- 0.0087 | 0.5054 +/- 0.0077 | 0.4448 +/- 0.0076 | 93 |

For reference, popularity NDCG@5 is `0.8871 / 0.7038 / 0.6544 / 0.5863`
for `m=1/3/9/19`. Qwen3.6-27B direct history listwise is therefore slightly
higher in all four slices. Qwen3-8B direct history remains below popularity and
has substantial strict-format attrition (81.48-86.29% test row coverage versus
99.81-99.92% for Qwen3.6-27B). Agent4Rec listwise remains below direct history
for Qwen3.6-27B; its test row coverage is 90.80-91.26% for Qwen3-8B and
99.77-99.88% for Qwen3.6-27B.

The matched Agent4Rec pointwise control below uses strict
`test_failure_as_negative.macro_by_user_group_mean.f1`. No-summary controls
were run only for yes/no, not for listwise ranking.

| backbone | candidate summary | m=1 | m=3 | m=9 | m=19 | failed groups |
|---|---|---:|---:|---:|---:|---:|
| Qwen3-8B | yes | 0.3807 +/- 0.0034 | 0.2638 +/- 0.0063 | 0.1407 +/- 0.0007 | 0.0856 +/- 0.0017 | 17,696 |
| Qwen3-8B | no | 0.4663 +/- 0.0061 | 0.3817 +/- 0.0039 | 0.2484 +/- 0.0034 | 0.1610 +/- 0.0022 | 77 |
| Qwen3.6-27B | yes | 0.5294 +/- 0.0040 | 0.4511 +/- 0.0056 | 0.2928 +/- 0.0022 | 0.1906 +/- 0.0036 | 841 |
| Qwen3.6-27B | no | 0.5391 +/- 0.0081 | 0.4548 +/- 0.0051 | 0.3053 +/- 0.0042 | 0.2015 +/- 0.0027 | 368 |

Candidate summaries reduce strict F1 and format adherence for Agent4Rec
interaction yes/no, especially for Qwen3-8B: test row coverage falls from
99.79-99.88% without summaries to 56.59-75.12% with summaries. The effect is
smaller but still negative for Qwen3.6-27B (99.07-99.56% without summaries
versus 97.92-98.61% with summaries).

Run roots:

- History listwise Qwen3-8B:
  `outputs/in_distribution/interaction_prediction/20260716T165403Z_qwen3_8b_ml1m_history_listwise`.
- History listwise Qwen3.6-27B:
  `outputs/in_distribution/interaction_prediction/20260716T165403Z_qwen36_27b_ml1m_history_listwise`.
- Agent4Rec candidate-summary Qwen3-8B:
  `outputs/in_distribution/interaction_prediction/20260718T0845Z_agent4rec_qwen3_8b_vk_proxy_traits_taste_candidate_summary_full`.
- Agent4Rec candidate-summary Qwen3.6-27B:
  `outputs/in_distribution/interaction_prediction/20260718T1605Z_agent4rec_qwen36_27b_traits_taste_candidate_summary_vk_proxy_full`.
- Agent4Rec no-summary Qwen3-8B:
  `outputs/in_distribution/interaction_prediction/20260719T0945Z_agent4rec_qwen3_8b_traits_taste_no_summary_control`.
- Agent4Rec no-summary Qwen3.6-27B:
  `outputs/in_distribution/interaction_prediction/20260720T0715Z_agent4rec_qwen36_27b_traits_taste_no_summary_control`.
...
- Direct LLM yes/no uses grouped hard decisions with at most 20 train-history
  ...
  statistics. The current ML-1M extension also evaluates a direct listwise
  prompt and Agent4Rec `traits+taste` profiles.
...
- The July 13 rows above evaluate the direct preference-specific yes/no prompt;
  the matched ML-1M history/listwise and Agent4Rec extension is reported below.
...
### Current ML-1M history/listwise and Agent4Rec extension

The July 16 direct-history rerun uses the same current preference tasks and
candidate rows as the Agent4Rec runs. Direct history exposes title, genres, and
the user's observed rating history, with no summaries or item statistics.
Agent4Rec uses `traits+taste`, with `gpt-4o-mini` taste profiles reused from the
same persistent caches as interaction prediction. Candidate-summary prompts add
`item_summary`; the no-summary controls show title and genres only. All variants
use identical task rows within a ratio and seed.

The direct yes/no rerun reproduces the earlier ML-1M result closely. Strict F1
is `test_failure_as_negative.macro_by_user_group_mean.f1`.

| method | backbone | m=1 | m=2 | m=3 | m=9 | failed groups |
|---|---|---:|---:|---:|---:|---:|
| History yes/no | Qwen3-8B | 0.2812 +/- 0.0067 | 0.2312 +/- 0.0115 | 0.1801 +/- 0.0060 | 0.0936 +/- 0.0014 | 0 |
| History yes/no | Qwen3.6-27B | 0.5965 +/- 0.0012 | 0.4919 +/- 0.0032 | 0.4150 +/- 0.0025 | 0.2282 +/- 0.0046 | 0 |
| Agent4Rec traits+taste, candidate summary | Qwen3-8B | 0.4123 +/- 0.0123 | 0.3509 +/- 0.0048 | 0.3029 +/- 0.0018 | 0.1869 +/- 0.0072 | 906 |
| Agent4Rec traits+taste, no summary | Qwen3-8B | 0.3996 +/- 0.0043 | 0.3379 +/- 0.0098 | 0.2955 +/- 0.0058 | 0.1768 +/- 0.0076 | 209 |
| Agent4Rec traits+taste, candidate summary | Qwen3.6-27B | 0.5705 +/- 0.0051 | 0.4784 +/- 0.0072 | 0.4070 +/- 0.0073 | 0.2236 +/- 0.0081 | 13 |
| Agent4Rec traits+taste, no summary | Qwen3.6-27B | 0.5686 +/- 0.0031 | 0.4736 +/- 0.0008 | 0.4129 +/- 0.0087 | 0.2292 +/- 0.0088 | 13 |

Candidate summaries modestly improve strict F1 for Qwen3-8B preference yes/no
despite worse format adherence: 906 failed test groups with summaries versus
209 without them. For Qwen3.6-27B the F1 effect is mixed and both variants have
only 13 failed groups. Thus the severe Qwen3-8B summary failure observed for
interaction is not a universal summary effect across targets.

Strict listwise NDCG@5 uses failed-group-zero evaluation. No-summary Agent4Rec
controls were not run for listwise ranking.

| method | backbone | candidate summary | m=1 | m=2 | m=3 | m=9 | failed groups |
|---|---|---|---:|---:|---:|---:|---:|
| Popularity ranking | -- | no | 0.7199 +/- 0.0013 | 0.6735 +/- 0.0091 | 0.6505 +/- 0.0045 | 0.4862 +/- 0.0090 | 0 |
| History listwise | Qwen3-8B | no | 0.6171 +/- 0.0053 | 0.5607 +/- 0.0130 | 0.5258 +/- 0.0120 | 0.3273 +/- 0.0112 | 410 |
| History listwise | Qwen3.6-27B | no | 0.7020 +/- 0.0039 | 0.6508 +/- 0.0088 | 0.6189 +/- 0.0081 | 0.4504 +/- 0.0064 | 11 |
| Agent4Rec traits+taste listwise | Qwen3-8B | yes | 0.6484 +/- 0.0041 | 0.5911 +/- 0.0143 | 0.5603 +/- 0.0145 | 0.3884 +/- 0.0137 | 114 |
| Agent4Rec traits+taste listwise | Qwen3.6-27B | yes | 0.6766 +/- 0.0075 | 0.6232 +/- 0.0138 | 0.5954 +/- 0.0092 | 0.4255 +/- 0.0166 | 0 |

Popularity remains strongest in all four preference-ranking slices. Agent4Rec
candidate summaries improve Qwen3-8B over direct history listwise, but reduce
Qwen3.6-27B relative to direct history. Test row coverage is 97.76-99.25% for
Qwen3-8B history, 99.90-100% for Qwen3.6-27B history, 99.48-99.75% for
Qwen3-8B Agent4Rec, and 100% for Qwen3.6-27B Agent4Rec.

Run roots:

- History Qwen3-8B:
  `outputs/in_distribution/preference_prediction/20260716T165403Z_qwen3_8b_ml1m_history`.
- History Qwen3.6-27B:
  `outputs/in_distribution/preference_prediction/20260716T165403Z_qwen36_27b_ml1m_history`.
- Agent4Rec candidate-summary Qwen3-8B:
  `outputs/in_distribution/preference_prediction/20260718T085211Z_agent4rec_qwen3_8b_traits_taste_candidate_summary_vk_proxy_full`.
- Agent4Rec candidate-summary Qwen3.6-27B:
  `outputs/in_distribution/preference_prediction/20260718T1605Z_agent4rec_qwen36_27b_traits_taste_candidate_summary_vk_proxy_full`.
- Agent4Rec no-summary Qwen3-8B:
  `outputs/in_distribution/preference_prediction/20260720T1600Z_agent4rec_qwen3_8b_traits_taste_no_summary_control`.
- Agent4Rec no-summary Qwen3.6-27B:
  `outputs/in_distribution/preference_prediction/20260720T0945Z_agent4rec_qwen36_27b_traits_taste_no_summary_control`.
